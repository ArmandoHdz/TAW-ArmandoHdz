...
class Datos extends Conexion{
	...
	public function cargarProductoModel($codigo){

		$stmt = Conexion::conectar()->prepare("SELECT * FROM products WHERE codigo_producto = :codigo");
		...
		if($stmt->fetchColumn() > 0){
			...
			if($cantidad >= 1){
				$stmt = Conexion::conectar()->prepare("UPDATE carga_producto SET cantidad = cantidad + 1, total = precio*cantidad WHERE codigo = :codigo");
				$stmt->bindParam(':codigo', $codigo, PDO::PARAM_STR);
				$stmt->execute();
			}
			else{
			...
			$stmt2 = Conexion::conectar()->prepare("INSERT INTO carga_producto(id, codigo, descripcion, precio, cantidad, total) VALUES(:id, :codigo, :descripcion, :precio, 1, :total)");
